...
static servo_err_t servo_pwm_set_compare(servo_t const *servo,
                                         uint16_t compare) {
  return servo->interface.pwm_set_compare
             ? servo->interface.pwm_set_compare(servo->interface.pwm_user,
                                                compare)
             : SERVO_ERR_NULL;
}
...
static uint16_t servo_angle_to_compare(servo_t const *servo, float32_t angle) {
  ...
  uint16_t compare =
      (uint16_t)((angle - servo->config.min_angle) *
                     (float32_t)(servo->config.max_compare -
                                 servo->config.min_compare) /
...
servo_err_t servo_initialize(servo_t *servo, servo_config_t const *config,
...
servo_err_t servo_set_angle_min(servo_t const *servo) {
  ...
  return servo_set_angle(
      servo, (servo->config.max_angle - servo->config.min_angle) / 2.0F);
}