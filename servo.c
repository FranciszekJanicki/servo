...
static servo_err_t servo_pwm_init(servo_t const* servo)
{
    return servo->interface.pwm_init ? servo->interface.pwm_init(servo->interface.pwm_user)
                                     : SERVO_ERR_NULL;
}

static servo_err_t servo_pwm_deinit(servo_t const* servo)
{
    return servo->interface.pwm_deinit ? servo->interface.pwm_deinit(servo->interface.pwm_user)
                                       : SERVO_ERR_NULL;
}
...
servo_err_t servo_deinitialize(servo_t* servo)
{
    ...
    servo_err_t err = servo_pwm_deinit(servo);
